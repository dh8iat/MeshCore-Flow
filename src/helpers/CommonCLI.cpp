  ...
  if (strcmp(command, prefix) == 0) {
    ...
    *dp++ = '>';
    ...
    for (uint16_t value = 0; value < 256; value++) {
      if (bitset[value >> 3] & (1 << (value & 7))) {
        if (any) *dp++ = ' ';
        sprintf(dp, "%02X", value);
        dp += 2;
        any = true;
      }
    }
    if (!any) strcpy(dp, " empty"); else *dp = 0;
    return true;
  }
...
static bool handleAnyBlacklistCommand(const char* command, NodePrefs* prefs, char* reply, bool* changed) {
  return handleBlacklistCommand(command, "sender", prefs->blk_sender, reply, changed) ||
         handleBlacklistCommand(command, "neighbor", prefs->blk_neighbor, reply, changed) ||
         handleBlacklistCommand(command, "channel", prefs->blk_channel, reply, changed);
}
...
void CommonCLI::loadPrefsInt(FILESYSTEM* fs, const char* filename) {
  ...
  if (file) {
    ...
    file.read((uint8_t *)&_prefs->flood_response_base, sizeof(_prefs->flood_response_base));   // 293
    file.read((uint8_t *)&_prefs->flood_request_base, sizeof(_prefs->flood_request_base));     // 297
    file.read((uint8_t *)_prefs->blk_neighbor, sizeof(_prefs->blk_neighbor));                  // 301
    file.read((uint8_t *)_prefs->blk_sender, sizeof(_prefs->blk_sender));                      // 333
    file.read((uint8_t *)_prefs->blk_channel, sizeof(_prefs->blk_channel));                    // 365
    // next: 397
...
void CommonCLI::savePrefs(FILESYSTEM* fs) {
  ...
  if (file) {
    ...
    file.write((uint8_t *)&_prefs->flood_response_base, sizeof(_prefs->flood_response_base));  // 293
    file.write((uint8_t *)&_prefs->flood_request_base, sizeof(_prefs->flood_request_base));    // 297
    file.write((uint8_t *)_prefs->blk_neighbor, sizeof(_prefs->blk_neighbor));                 // 301
    file.write((uint8_t *)_prefs->blk_sender, sizeof(_prefs->blk_sender));                     // 333
    file.write((uint8_t *)_prefs->blk_channel, sizeof(_prefs->blk_channel));                   // 365
    // next: 397
...
void CommonCLI::handleCommand(uint32_t sender_timestamp, char* command, char* reply) {
    if (memcmp(command, "poweroff", 8) == 0 || memcmp(command, "shutdown", 8) == 0) {
      ...
      bool changed = false;
      if (handleAnyBlacklistCommand(command, _prefs, reply, &changed)) {
...
void CommonCLI::handleSetCmd(uint32_t sender_timestamp, char* command, char* reply) {
  ...
  if (memcmp(config, "dutycycle ", 10) == 0) {
    ...
    if (f >= 0.0f && f <= 1.0f) {
      ...
      strcpy(reply, "Error, must be 0.0-1.0");
    }
  } else if (memcmp(config, "flood.req.base ", 15) == 0) {
...
void CommonCLI::handleGetCmd(uint32_t sender_timestamp, char* command, char* reply) {
  ...
  } else if (memcmp(config, "flood.req.base", 14) == 0) {
